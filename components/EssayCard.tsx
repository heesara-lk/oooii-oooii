...
export function EssayCard({ essay, isRead, isNew, isLiked, likeCount, isFreePick, onOpen, onLike }: Props) {
  return (
    <div onClick={onOpen} className={`group cursor-pointer paper rounded-[18px] overflow-hidden border book-shadow hover:shadow-[0_24px_40px_-18px_rgba(0,0,0,0.4)] transition-all ${isRead ? 'opacity-[0.85]' : ''} border-[#e9ddd0]`}>
      ...
      <div className="p-5">
        ...
        <div className="flex items-center justify-between mt-4">
          <button onClick={(e)=>{ e.stopPropagation(); onLike?.(); }} className={`text-[12px] px-2 py-1 rounded-full border transition ${isLiked ? 'bg-[#c9a86a] text-white border-[#c9a86a]' : 'bg-[#fdf6ec] border-[#e9ddd0] text-[#9a8470] hover:bg-[#f5efe0]'}`}>♥ {likeCount ?? essay.likes}</button>
          <span className="text-[11px] uppercase tracking-[0.16em] text-[#9a8470]">Read →</span>
        </div>
      </div>
    </div>
  );
