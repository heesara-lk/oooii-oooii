    ...
    <header className="sticky top-0 z-20 backdrop-blur bg-[#f6efe2]/80 border-b border-[#e9ddd0]">
      <div className="max-w-[1180px] mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {userEmail ? (
            <>
              <span className="hidden md:inline text-[11px] text-[#6b5a4a] max-w-[120px] truncate">{userEmail}</span>
          ...
          <button onClick={()=>onNav('admin')} className="text-[11px] uppercase tracking-[0.16em] ml-1">Admin</button>
        </div>
      </div>
    </header>
  );
