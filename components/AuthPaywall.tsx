...
export function AuthPaywall({ freePicks, onClose, onSubscribed, onAuth, currentUserEmail }: Props) {
  ...
  useEffect(() => {
    if (currentUserEmail) {
      setHasAccount(true);
      if (freePicks >= 5) setMode('pay');
      else setMode('otp_email');
    } else {
      setHasAccount(false);
      setMode('otp_email');
      if (freePicks >= 5) setMode('otp_email');
    }
  ...
  const handleSendOtp = async () => {
    ...
    try {
      ...
      setMsg(`OTP sent to ${email} - check inbox`);
      setMode('otp_verify');
  ...
  const handleMockPayment = async () => {
    ...
    if (error) { setErr("Payment failed: " + error.message); setLoading(false); return; }
      ...
      <div className="paper rounded-[20px] border border-[#e9ddd0] p-6 max-w-[440px] w-full book-shadow">
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-[12px] text-[#6b5a4a] mt-2">{isBefore ? "Step 1: OTP account (like heesara.lk) to get 3 more free" : "Step 2: Unlock lifetime - $29"}</p>
        ...
        {mode === 'otp_email' && (
          <>
            <div className="mt-5 p-3 rounded-[12px] bg-blue-50 border border-blue-200">
              ...
              <p className="text-[11px] text-blue-700 mt-1">No password needed</p>
            </div>
            ...
            <p className="mt-2 text-[10px] text-[#9a8470] text-center">Works on vercel.app today, and oooii.com/.lk later - no code change</p>
          </>
        )}
        ...
        {mode === 'otp_verify' && (
          <>
            <div className="mt-5 p-3 rounded-[12px] bg-blue-50 border border-blue-200">
              ...
              <p className="text-[11px] mt-1">Enter 6-digit code or click link</p>
            </div>
            ...
            {err && <p className="mt-2 text-[11px] text-red-600 bg-red-50 p-2 rounded">{err}</p>}
            {msg && <p className="mt-2 text-[11px] text-green-700 bg-green-50 p-2 rounded">{msg}</p>}
            <button onClick={handleVerifyOtp} disabled={loading} className="mt-4 w-full py-3 rounded-full bg-[#2D8A4E] text-white text-[11px] uppercase tracking-[0.16em] disabled:opacity-50">{loading ? 'Verifying...' : 'Verify OTP & Continue'}</button>
          ...
          <>
            <div className="mt-5 p-4 rounded-[12px] bg-[#fdf6ec] border border-[#e9ddd0]">
              ...
              <ul className="mt-2 text-[12px] text-[#6b5a4a] list-disc pl-4 space-y-1">
                <li>Account: {currentUserEmail || email}</li>
                <li>All past & future daily chapters</li>
                <li>Works on oooii.com/.lk when you buy domain</li>
              </ul>
            </div>
            {err && <p className="mt-2 text-[11px] text-red-600 bg-red-50 p-2 rounded">{err}</p>}
